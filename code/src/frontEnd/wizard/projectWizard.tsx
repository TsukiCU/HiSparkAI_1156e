...
const ProjectWizard = (): JSX.Element => {
  ...
  const onFinish = (): void => {
    form.validateFields().then(() => {
      dispatch(sendProjectData({
        ...
        projectData: { soc, board: soc, platform, projectName, projectPath, sdkPath },
      }));
    }).catch(() => {});
  };
  ...
  const allChips: SocChipItem[] = chipList.reduce<SocChipItem[]>((list, group) => list.concat(group.children ?? []), []);
  ...
  return (
    <>
      <Modal
        title={<span style={{ color: '#666' }}>{t('projectWizard')}</span>}
        visible={isOpen}
        width={600}
        onCancel={onCancel}
        footer={
      ...
      >
        ...
        <Form layout="vertical" form={form} autoComplete="off">
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item label={t('SOC')} name="soc" rules={[{ required: true, message: t('fieldCannotEmpty', { field: t('SOC') }) }]}>
            ...
            <Col span={12}>
              <Form.Item label={t('platform')} name="platform" rules={[{ required: true, message: t('fieldCannotEmpty', { field: t('platform') }) }]}>
                <Select value={platform || undefined} disabled={platformFixed || !soc} onChange={(v: 'CPU' | 'NPU') => setPlatform(v)} placeholder="—">
          ...
          <Row>
            <Col span={24}>
              <Form.Item label={t('sdkPath')} name="sdkPath" rules={[{ required: true, message: t('fieldCannotEmpty', { field: t('sdkPath') }) }]}>
                <Input.Group compact>
                  <Input readOnly style={{ width: 'calc(100% - 37px)' }} placeholder={t('sdkPathInputPrompt')} value={sdkPath} onClick={onBrowseSdkPath} />
                  <Button type="primary" style={{ paddingLeft: 10 }} onClick={onBrowseSdkPath} icon={<FolderOpenOutlined style={{ color: '#fff' }} />} />
                </Input.Group>
              </Form.Item>
            </Col>
          </Row>
          ...
          <Row>
            <Col span={24}>
              <Form.Item label={t('projectName')} name="projectName" rules={nameRules}>
                <Input placeholder={t('projectNameInputPrompt')} value={projectName} onChange={(e) => setProjectName(e.target.value)} />
              </Form.Item>
            </Col>
          </Row>
          ...
          <Row>
            <Col span={24}>
              <Form.Item label={t('projectPath')} name="projectPath" rules={pathRules}>
                <Input.Group compact>
                  <Input readOnly style={{ width: 'calc(100% - 37px)' }} placeholder={t('projectPathInputPrompt')} value={projectPath} onClick={onBrowsePath} />
                  <Button type="primary" style={{ paddingLeft: 10 }} onClick={onBrowsePath} icon={<FolderOpenOutlined style={{ color: '#fff' }} />} />
                </Input.Group>
              </Form.Item>
            </Col>
          </Row>
