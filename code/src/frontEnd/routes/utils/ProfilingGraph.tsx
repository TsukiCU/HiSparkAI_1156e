...
export interface ProfilingGraphData {
  x: number;
  y: number;
}
export interface ProfilingGraphs {
  xTitle?: string;
  yTitle?: string;
  width?: string;
  height?: string;
  overflowX?: CSSProperties['overflowX'];
  profilingGraphData: ProfilingGraphData[];
}
type EChartsOption = echarts.EChartsOption;
const fontFamily = 'Arial, "Microsoft YaHei", sans-serif';
// 使用 React.FC 定义组件类型
const profilingGraph: React.FC<ProfilingGraphs> = ({
  ...
  const chartOption = useMemo<EChartsOption>((): any => {
    return {
      ...
      xAxis: {
        ...
        nameGap: 25,
        textStyle: { fontFamily },
        data: profilingGraphData.map(item => (parseFloat(`${item.x}`)).toFixed(4)),
        axisLabel: {
          show: true,
          fontFamily,
        },
      ...
      yAxis: {
        ...
        splitLine: { show: true }, // 增加分隔线
      },
      series: [
        {
          data: profilingGraphData.map(item => {
            if (item.y > 1) {
              return { ...item, value: parseFloat(`${item.y / 100}`) }; // 实际柱子高度
            } else if (item.y === 0) {
              return { ...item, value: item.y }; // 实际柱子高度
            } else {
  ...
  }, [xTitle, yTitle, profilingGraphData]);
  ...
  }, [xTitle, yTitle, profilingGraphData, width]);
  ...
  return (
    <div style={{
      ...
      <div ref={chartRef} style={{ width, height, minHeight: overflowX === 'hidden' ? '200px' : '', position: 'relative', left: '-1px', top: '6px', display: 'inline-block' }}></div>
    </div>
  );
};
