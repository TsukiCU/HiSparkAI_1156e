 ...
 * chip config panel
 */
export default class ProjectSettingPanel implements Panel {
  ...
  toggle(): void {
    ...
    const projectSettingLightIcon = extension.isCustomIDE ?
      icon.customProjectSettingLight :
      icon.projectSettingLight;

    const projectSettingDarkIcon = extension.isCustomIDE ?
      icon.customProjectSettingDark :
      icon.projectSettingDark;

    this.panel.iconPath = {
      light: vscode.Uri.file(getResource.get(projectSettingLightIcon)),
      dark: vscode.Uri.file(getResource.get(projectSettingDarkIcon)),
    };
    ...
    this.panel.webview.html = fs
      ...
      .replace('flagdefault', 'flagSettings')
      .replace('{{path}}', this.path ? this.path : '')
      .toString();
    const ss = this.panel.webview.html;
  }
  ...
  newPanel(): WebviewPanel {
    const panel = vscode.window.createWebviewPanel('Project Settings', res('projectSettingName'), vscode.ViewColumn.One, {
      enableScripts: true,
