...
export default class ProjectImportPanel implements Panel {
  ...
  toggle(): void {
    ...
    this.panel.iconPath = {
      light: vscode.Uri.file(getResource.get(icon.importProjectLight)),
      dark: vscode.Uri.file(getResource.get(icon.importProjectDark)),
    };
    // 获取index.html的路径
  ...
  newPanel(): WebviewPanel {
    const panel = vscode.window.createWebviewPanel('Project Import', res('importProjectName'), vscode.ViewColumn.One, {
      enableScripts: true,
